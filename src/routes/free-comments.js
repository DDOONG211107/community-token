...
router.get(
  ...
  wrapper(async (req, res, next) => {
    ...
    const selectResult = await pgPool.query(
      `
        SELECT free_board.comment.*, account.list.nickname,
            CASE WHEN free_board.comment.account_idx = $1
            THEN true ELSE false END AS is_mine
        FROM free_board.comment 
        JOIN account.list 
        ON free_board.comment.account_idx = account.list.idx
...
router.post(
  ...
  wrapper(async (req, res, next) => {
    ...
    await pgPool.query(
      `
        INSERT INTO free_board.comment (content, list_idx, account_idx)
        VALUES ($1, $2, $3) RETURNING idx;
      `,
      [content, free_idx, accountIdx]
    );
...
router.put(
  ...
  wrapper(async (req, res, next) => {
    ...
    await pgPool.query(
      `
        UPDATE free_board.comment SET content = $1 WHERE
...
router.delete(
  ...
  wrapper(async (req, res, next) => {
    ...
    await pgPool.query(
      `
          DELETE FROM free_board.comment 
